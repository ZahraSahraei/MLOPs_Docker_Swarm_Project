...
def login():
    if request.method == 'POST':
        ...
        if user and email and bcrypt.check_password_hash(user.password, password):
...
def profile():
    ...
    return render_template('profile.html', data=data_list, feature_names=data.feature_names)
